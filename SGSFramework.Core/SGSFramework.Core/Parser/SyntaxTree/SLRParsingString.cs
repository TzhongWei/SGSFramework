...
using SGSFramework.Core.Parser.LRParseing.SLR;
using SGSFramework.Core.Parser.LRParseing.LALR;


namespace SGSFramework.Core.Parser.SyntaxTree
{
    public interface LR0ParsingString<TKernelItem> : IParsingString where TKernelItem : BaseLR0KernelItem, IEquatable<TKernelItem>
    { }
    public class SLRParsingString: LR0ParsingString<LR0KernelItem>
    {
        public string cmd => _cmd;
        private string _cmd = "================SLRParsingString============\n";
        public IParsingTable<BaseLR1ActionDictionary<LR0KernelItem>> parsingTable { get; }
        public string Sentence;
        public SyntaxTree Tree { get; set; }
        public SLRParsingString(string S, IParsingTable<BaseLR1ActionDictionary<LR0KernelItem>> parsingTable)
        {
            Sentence = S;
            this.parsingTable = parsingTable;
        }
        public SLRParsingString(string S, SLR1Parsing Parser):this(S, Parser.ParsingTable)
        {
        }
        public SyntaxTree StartParsing()
        {
            ...
            BaseLR1ActionDictionary<LR0KernelItem> foundRow;
            ...
            while (productionIndex != -1)
            {
                ...
                if (foundRow.TryGetValue(input, out var Action))
                {
                    ...
                    {
                        var Rules = Action.Reduce.First(); //SLR parser only accept one reduce
