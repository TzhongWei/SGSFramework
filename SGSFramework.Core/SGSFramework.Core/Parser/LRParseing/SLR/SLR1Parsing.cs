...
namespace SGSFramework.Core.Parser.LRParseing.SLR
{
    ...
    public class SLR1Parsing : BaseLR0Parsing<SLR1Action>
    {
        ...
        public override IParsingTable<SLR1Action> CreateParsingTable()
        {
            ...
            IDictionary<string, HashSet<string>> followset = this.cfgGrammar.ComputeFollowSet();
            string end = "$";
            SLR1Action actions;
            ParsingTable<SLR1Action> table = new ParsingTable<SLR1Action>();
            
            //Each table in the automaton can be seen as a row in the parsing table
            foreach (State<LR0KernelItem> state in automaton)
            {
                //Initialise a new action (it can be considered a row in the parsing table)
                actions = new SLR1Action();
                //Each transition is either a shift (on a terminal) or a goto (on a non-terminal)
                foreach (KeyValuePair<string, State<LR0KernelItem>> kv in state.Transitions)
                {
                    actions.Add(kv.Key, new LR1ActionItem<LR0KernelItem>(kv.Value));
                }
                //A reduction occurs when the end of a production has been reached
                foreach (LR0KernelItem item in state.Items)
                {
                    //The null production (start token), has a length of 0. If the end of 
                    //this "production" has been reached at the end of  the input ($) the
                    //input ($) the input can be accepted
                    ...
                    else
                    {
                        ...
                        if (item.Index == item.Production.Body.Count)
                        {
                            ...
                            //If the end of the production has been reached,
                            //reduce to the follow set of the head (a non-terminal),
                            //the lookahead. this should be reduce parsing table size.
                            foreach (string token in followset[item.Production.Head])
                            {
