...
namespace SGSFramework.Core.Parser.LRParseing
{
    ...
    public class LR0Parsing : BaseLR0Parsing<LR0Action>
    {
        ...
        public override void Classify()
        {
            ...
            ParsingTable<LR0Action> table = this.CreateParsingTable() as ParsingTable<LR0Action>;
            HashSet<string> terminals = this.cfgGrammar.ComputeTerminals();
            ...
            foreach (LR0Action action in table)
            {
                ...
                {
                    foreach (string s in action.Shift.Keys)
                    {
        ...
        public override IParsingTable<LR0Action> CreateParsingTable()
        {
            ...
            foreach (State<LR0KernelItem> state in automation)
            {
                ...
                foreach (LR0KernelItem item in state.Items)
                {
                    ...
                    //If the end of a production body has been reached, a reduction
                    //to the head (a non-terminal) occurs
                    else
                    {
    ...
    public class LR0ClassificationException : Exception
    {
