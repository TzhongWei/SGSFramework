    ...
    */
    public class LALR1Parsing : BaseLR1Parsing<LALR1Action, LALR1KernelItem>
    {
        ...
        public override IAutomaton<LALR1KernelItem> CreateAutomaton()
        {
            ...
            IAutomaton<LALR1KernelItem> lr1Automaton = base.CreateAutomaton();

            //Initialise temporary items for merging
            ...
            Dictionary<State<LALR1KernelItem>, State<LALR1KernelItem>> translation =
                new Dictionary<State<LALR1KernelItem>, State<LALR1KernelItem>>();
            ...
            foreach (State<LALR1KernelItem> lr1State in lr1Automaton)
            {
                if (translation.TryGetValue(lr1State, out intermediateState))
                {
                    if (lr1State.Transitions is null)
                        continue;
                    intermediateState.Transitions = new Dictionary<string, State<LALR1KernelItem>>();

                    //Lookup the correct new state, and add as transition
                    foreach(KeyValuePair<string, State<LALR1KernelItem>> kv
            ...
            IDictionary<string, State<LALR1KernelItem>> original;
            ...
            for (int i = 0; i < merge.Count; i++)
            {
                ...
                state.Transitions
                    = new Dictionary<string, State<LALR1KernelItem>>();
        ...
        public override IParsingTable<LALR1Action> CreateParsingTable()
        {
            ...
            LALR1Action actions;
            string end = "$"; //The end of the token string, add in the look ahead list

            //Each state in the automaton can be seen as a row in the paring table
        ...
        /// <returns></returns>
        private static Kernel<LALR1KernelItem> CollapseLookaheads(IKernel<BaseLR1KernelItem> items)
        {
            ...
            foreach (BaseLR1KernelItem item in items)
            {
                ...
                if (!result.Contains((BaseLR0KernelItem)item, out lalr1KernelItem))
                {
                    result.Add(lalr1KernelItem = new LALR1KernelItem(item));
                }
        ...
        /// <param name="b"></param>
        /// <returns></returns>
        private static Kernel<LALR1KernelItem> MergeItems(Kernel<LALR1KernelItem> a, Kernel<LALR1KernelItem> b)
        {
            Kernel<LALR1KernelItem> result = new Kernel<LALR1KernelItem>();
            LALR1KernelItem kernelItem;

            //Return new kernek with all items from a
            if (b is null)
            {
            ...
            else
            {
                ...
                foreach (LALR1KernelItem innerKernelItem in b)
                {
                    ...
                    {
                        foreach (string token in innerKernelItem.LookAheads)
                        {
        ...
        {
            if (this.ParsingTable == null)
                return base.ToString() + " parsing table is null";
            string Result = this.GetType().Name + "\n";
            Result += Display.TableDisplay.PrintLR1Table(this.ParsingTable);
            return Result;
        }
    }
}
